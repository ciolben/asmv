...
void Decode_Picture(int bitstream_framenum, int sequence_framenum, int skip)

{
  ...
  //  cout << " PICTURE END "<<endl;
  /* write or display current or previously decoded reference frame */
  /* ISO/IEC 13818-2 section 6.1.1.11: Frame reordering */
  //  cout <<" First_Frame " << First_Frame << " " << Last_Frame<<endl;
  if (skip==0)
    frame_reorder(bitstream_framenum, sequence_framenum);

  //  cout<< " pic "<<bitstream_framenum<< " "<<sequence_framenum<< endl;
  if ((sequence_framenum/100)*100==sequence_framenum)
    cout <<" first " << sequence_framenum <<" images decoded"<<endl;
...

{
  ...
  //	cout <<"  BEGstart of slice ==========================================="<<endl;
  for(;;)
  {

    if((ret=slice(framenum, MBAmax))<0)
      {
	//	cout <<"  End statr of slice ==========================================="<<endl;
	return;
      }
...
static void frame_reorder(int Bitstream_Framenum, int Sequence_Framenum)
{
  ...
  //  cout <<" Sequence_Frame "<<Sequence_Framenum<<endl;
...
static int start_of_slice(int MBAmax,int * MBA, int *MBAinc,
