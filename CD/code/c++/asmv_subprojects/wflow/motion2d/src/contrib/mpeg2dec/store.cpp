...
/* private prototypes */
static void store_ppm( unsigned char *src[],
...
extern unsigned char *imaptr;

/*
 * store a picture as either one frame or two fields
 */
void Write_Frame(unsigned char *src[],
		 int frame)
{

  if (progressive_sequence || progressive_frame || Frame_Store_Flag)
  {
    store_ppm(src,0,Coded_Picture_Width,vertical_size);
    //store_one(Output_Picture_Filename,src,0,Coded_Picture_Width,vertical_size);
    return;
  }
  store_ppm(src,0,Coded_Picture_Width<<1,vertical_size>>1);


  store_ppm(src,   Coded_Picture_Width,Coded_Picture_Width<<1,vertical_size>>1);
		...
		int height)
{
  ...
  for (i=0; i<height; i++)
  {
	...
	if (imaptr != NULL)
	  {
	    *imaptr++ = (unsigned char) r;
	    *imaptr++ = (unsigned char) g;
	    *imaptr++ = (unsigned char) b;
	  }
