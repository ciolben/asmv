...
void KinemComp(IplImage* flow, DescMat* descMatcurldiv, DescMat* descMatshearC, DescMat* descMatshearD, DescInfo curldivInfo, DescInfo shearCDInfo)
{
	int width = descMatcurldiv->width;
	int height = descMatcurldiv->height;
	IplImage* flowX = cvCreateImage(cvSize(width,height), IPL_DEPTH_32F, 1);
        IplImage* flowY = cvCreateImage(cvSize(width,height), IPL_DEPTH_32F, 1);
        IplImage* flowXdX = cvCreateImage(cvSize(width,height), IPL_DEPTH_32F, 1);
        IplImage* flowXdY = cvCreateImage(cvSize(width,height), IPL_DEPTH_32F, 1);
	...
	for(int i = 0; i < height; i++) {
        ...
        BuildDescMat(divImg, curlImg, descMatcurldiv, curldivInfo);
        BuildDescMat(curlImg, shearImg, descMatshearC, shearCDInfo);
        BuildDescMat(divImg, shearImg, descMatshearD, shearCDInfo);
