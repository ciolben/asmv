...
public:
    ...
    virtual bool imwrite(QImage *imgout, ImageIO::ImageType imagetype, int quality=100) const;
    virtual bool imwrite(const QString& filename) const;
    virtual bool imwrite(QImage *imgout) const;
...
template <class T>
bool Image<T>::imwrite(QImage* imgout, ImageIO::ImageType imagetype, int quality) const
{
    return ImageIO::writeImage(NULL,(const T*&)pData,imWidth,imHeight,nChannels,imagetype,quality, imgout);
}

template <class T>
bool Image<T>::imwrite(const QString &filename) const
{
    ImageIO::ImageType type;
    qDebug() << "filename";
    if(IsDerivativeImage)
        ...
        type=ImageIO::standard;
    return ImageIO::writeImage(filename,(const T*&)pData,imWidth,imHeight,nChannels,type, 100);
}

template <class T>
bool Image<T>::imwrite(QImage* imgout) const
{
    ImageIO::ImageType type;
    qDebug() << "imgout";
    if(IsDerivativeImage)
        ...
        type=ImageIO::standard;
    return ImageIO::writeImage(NULL,(const T*&)pData,imWidth,imHeight,nChannels,type, 100, imgout);
}
