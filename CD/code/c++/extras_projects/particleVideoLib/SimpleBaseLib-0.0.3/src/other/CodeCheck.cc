...
void checkCodeFormatting( Config &conf ) {
    ...
    const String &path = conf.readString( "path" ); // note: no need to add data path
...
void distributeCode( Config &conf ) {
    ...
    // get command parameters
    const String &sourcePath = conf.readString( "sourcePath" ); // note: no need to add data path
    const String &destPath = conf.readString( "destPath" );
    Array<String> suffixes = conf.readStrings( "suffixes" );
    bool excludeUnderscore = conf.readBool( "excludeUnderscore", true );
    if (conf.initialPass())
        return;
