...
public:
	...
	template <class T>
    static bool writeImage(const QString& filename, const T*& pImagePlane,int width
                           ,int height,int nchannels,ImageType type=standard,int quality=-1, QImage* out = 0);

	template <class T>
    static bool writeImage(const QString& filename,const T* pImagePlane,int width
                           ,int height,int nchannels,T min, T max,int quality=-1, QImage* out = 0);
...
template <class T>
bool ImageIO::writeImage(const QString& filename, const T*& pImagePlane,int width
                         ,int height,int nchannels,ImageType type,int quality, QImage* out)
{
    ...
    bool result;
    if(out == 0) {
        result = pQImage->save(filename,0,quality);
        delete pQImage;
    } else {
        *out = *pQImage;
        result = true;
...
template <class T>
bool ImageIO::writeImage(const QString& filename, const T* pImagePlane, int width
                         , int height, int nchannels, T min, T max, int quality, QImage *out)
{
	int nPixels=width*height,nElements;
	nElements=nPixels*nchannels;
	unsigned char* pTempBuffer;
