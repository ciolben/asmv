...
/* Note: fields are only read to serve as the same-frame reference for
   a second field */
static void Read_Frame(char *fname,
		       unsigned char *frame[],
		       int framenum
		       )
{
...
static int Read_Component(char *Filename,
			  unsigned char *Frame,
			  int Width,
			  int Height
			  )
{
  ...
  Bytes_Read = fread(Frame, sizeof(int),Size,Infile);
...
static void Copy_Frame(unsigned char *src,
		       unsigned char *dst,
		       int width,
		       int height,
