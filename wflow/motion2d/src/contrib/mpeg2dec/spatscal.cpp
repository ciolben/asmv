 ...
 */
static void Deinterlace(unsigned char *fld0,
			unsigned char*fld1,
			int j0,
			int lx,int ly, /* ly has to be even */
			int aperture
			)
{
  int i,j,v;
...
/* vertical resampling */
static void Subsample_Vertical(unsigned char *s,
			       short *d,
			       int lx, int lys,int  lyd, int m,int  n,int  j0,int  dj)
{
  int i, j, c1, c2, jd;
...
static void Subsample_Horizontal(short *s,
				 unsigned char *d,
				 int x0, int lx, int lxs,int lxd,int ly,int m, int n)
