...
#include "exttypes/mvectors.h"
#include "exttypes/dct.h"

#include "mpeg2dec.h"

//using namespace std;

// Precision des vecteurs
extern int VPRECF, VPRECB;

unsigned char *imaptr;
TVectorTable *Forward;
TVectorTable *Backward;
TVectorTable *ForwardRef;
TVectorTable *BackwardRef;
...
static void Process_Options (const char *filename);
#ifdef OLD
static void Process_Options_Old (int argc, char *argv[]);
#endif
int Bitstream_Framenumber = 0;
int Sequence_Framenum0 = 0;
...
int OpenMPEG(const char *filename, int *height, int *width)
{
  int ret, code;
  Clear_Options();
  ...
  base.Infile = fopen(Main_Bitstream_Filename,"rb");
  if (base.Infile == NULL)
  {
  ...
  if(Two_Streams)
  {
    ...
    if ((enhan.Infile = fopen(Enhancement_Layer_Bitstream_Filename,"rb"))<0)
    {
...
int CloseMPEG()
{
  VectorTableFree(ForwardRef, 1);
  VectorTableFree(BackwardRef, 1);
...
int InitMPEG(int getdct)
{
  ForwardRef =  VectorTableAlloc( ForwardRef, mb_height, mb_width, 1, 1);
  BackwardRef =  VectorTableAlloc( BackwardRef, mb_height, mb_width, 1, 1);
  ForwardRef->nbli = BackwardRef->nbli = mb_height;
  ForwardRef->nbco = BackwardRef->nbco = mb_width;
  Forward = ForwardRef;
  Backward = BackwardRef;
  if (getdct){
    dctRef =  dctTableAlloc(dctRef, 2*mb_height, 2*mb_width, 1, 1);
    dc_dct = dctTableAlloc(dc_dct, 2*mb_height, 2*mb_width, 1, 1);
    CleardctTable(dc_dct);
    CopydctTable(dc_dct, dctRef);

  }
  Forward->vprec = 2.0-VPRECF;
  Backward->vprec = 2.0-VPRECB;
  return 1;
...
int GetMPEGFrame(unsigned char *pixels, int skip)
{
  int Return_Value;
  int Bitstream_Framenum = Bitstream_Framenumber;
  imaptr = pixels;

  Decode_Picture(Bitstream_Framenum, Sequence_Framenum0,skip);
  //cout <<" get Frame "<< Bitstream_Framenum<< " " << Sequence_Framenum0<< " " << Second_Field<<endl;
...
int GetMPEGVectors(TVectorTable *forward, TVectorTable *backward)
{
  int Return_Value;
  int Bitstream_Framenum = Bitstream_Framenumber;

  TVectorTable *tmpforward;
  TVectorTable *tmpbackward;
  ...
  tmpforward = NULL;
  tmpbackward = NULL;
  ClearVectorTable(Forward);
  ClearVectorTable(Backward);
  Forward->vprec = 2.0-VPRECF;
  Backward->vprec = 2.0-VPRECB;

  Decode_Picture(Bitstream_Framenum, Sequence_Framenum0);
  // Réarangement des images
  if (Forward->type != BTYPE){

    tmpforward =  VectorTableAlloc( tmpforward, mb_height, mb_width, 1, 1);
    tmpbackward =  VectorTableAlloc( tmpbackward, mb_height, mb_width, 1, 1);
    tmpforward->nbli = tmpbackward->nbli = mb_height;
    tmpforward->nbco = tmpbackward->nbco = mb_width;

    CopyVectorTable(Forward, tmpforward);
    CopyVectorTable(ForwardRef, Forward);
    CopyVectorTable(tmpforward, ForwardRef);

    CopyVectorTable(Backward, tmpbackward);
    CopyVectorTable(BackwardRef, Backward);
    CopyVectorTable(tmpbackward, BackwardRef);

    VectorTableFree(tmpforward, 1);
    VectorTableFree(tmpbackward, 1);
...
int GetMPEGData(TVectorTable *forward, TVectorTable *backward, TdctTable *dct)
{
  int Return_Value;

  CleardctTable(dc_dct);

  Return_Value = GetMPEGVectors(forward, backward);

  // Réarangement des images de dct
  if (forward->type != BTYPE){
    CopydctTable(dctRef, dct);
    CopydctTable(dc_dct, dctRef);
  }
  else{

      CopydctTable(dc_dct, dct);
  }
...
/* option processing */
static void Process_Options(const char *filename)              /* argument vector */
{
  ...
  //sprintf(Main_Bitstream_Filename, "%s", filename);
  sprintf(tmp, "%s", filename);
  Main_Bitstream_Filename = tmp;
  //sprintf(Output_Picture_Filename,"currentimage");
  Output_Picture_Filename = "currentimage%d.ppm";
  //Verbose_Flag = 3;
}

#ifdef OLD
/* option processing */
static void Process_Options_Old(int argc,                  /* argument count  */
